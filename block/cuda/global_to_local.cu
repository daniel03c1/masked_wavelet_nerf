...
__global__ void global_to_local_kernel(float* points, const float* domain_mins, const float* domain_maxs, const int64_t* batch_size_per_network, const int64_t* offsets) {
    ...
    while (idx < 3 *  batch_size_per_network[blockIdx.y]) {
        ...
        // if (blockIdx.y >= 2031 && blockIdx.y <= 2032) {
        //     printf("block: %d, original pts: %f, min: %f, max: %f, size: %f res: %f\n", blockIdx.y, tmp, domain_min[coord_dim], \
        //     domain_max[coord_dim], domain_max[coord_dim] - domain_min[coord_dim],  points[offset + idx]);
        // }
    }
}
...
void global_to_local_cuda(const torch::Tensor& points_tensor, const torch::Tensor& domain_mins_tensor, const torch::Tensor& domain_maxs_tensor,
    ...
    // std::cout<<*points<<" "<<*(points+4)<<" "<<*(points+8)<<" "<< std::endl;
    ...
    cudaStream_t stream = at::cuda::getCurrentCUDAStream(); // grid = 1, num_net    block = 64
