...
def getMask(smallSize, largeSize):
    mask = torch.zeros(largeSize).to(torch.bool)
    mask[:smallSize[0], :smallSize[1]] = True
    assert np.array(smallSize).prod() == mask.sum()
    return mask


def grid_sample_cmplx(input, grid, mode='bilinear',
...
def grid_sample(inputs, grid, mode='bilinear', padding_mode='zeros',
                align_corners=True):
    return F.grid_sample(inputs, grid, mode, padding_mode, align_corners)
