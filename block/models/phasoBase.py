...
class PhasorBase(torch.nn.Module):
    ...
    def save(self, path):
        ...
        if self.alphaMask is not None:
            alpha_volume = self.alphaMask.alpha_volume_block.bool().cpu().numpy()
            ckpt.update({'alphaMask.shape':alpha_volume.shape})
            ckpt.update({'alphaMask.mask':np.packbits(alpha_volume.reshape(-1))})
            ckpt.update({'alphaMask.aabb': self.alphaMask.aabb.cpu()})
            ckpt.update({'alphaMask.domain_min': self.alphaMask.domain_min.cpu()})
            ckpt.update({'alphaMask.domain_max': self.alphaMask.domain_max.cpu()})
    ...
    def load(self, ckpt):   # issue 여기서 에러
        if 'alphaMask.aabb' in ckpt.keys():
            ...
            self.alphaMask = AlphaGridMask(self.device, ckpt['alphaMask.aabb'].to(self.device), alpha_volume.float().to(self.device), 
                    ckpt['kwargs']['block_split'], ckpt['alphaMask.domain_min'].to(self.device), ckpt['alphaMask.domain_max'].to(self.device))
    ...
    @torch.no_grad()
    def updateAlphaMask(self, domain_min, domain_max, gridSize=(200,200,200)):

        alpha, dense_xyz = self.getDenseAlpha(gridSize) # Fine
        ...
        if block:
            ...
            for x in range(bs_x):
                for y in range(bs_y):
                    for z in range(bs_z):
                        owow = alpha[0:1, 0:1, x*X : (x+1)*X, y*Y : (y+1)*Y, z*Z : (z+1)*Z]
                        owow = F.max_pool3d(owow, kernel_size=ks, padding=ks // 2, stride=1).view(X,Y,Z)
                        new_alpha[x*X : (x+1)*X, y*Y : (y+1)*Y, z*Z : (z+1)*Z] = owow
    ...
    def forward(self, rays_chunk, white_bg=True, is_train=False, ndc_ray=False, N_samples=-1, vis_normal=False):
        ...
        # 근데 알파마스킹까지 하니까 더 애매하네;
            ...
            sigma_feature = self.compute_densityfeature(xyz_sampled[ray_valid]) # 2K
            validsigma = self.feature2density(sigma_feature)
